...
export function CanvasVoiceNote({ block, zoom }: { block: CustomBlock; zoom: number }) {
  ...
  return <div className="relative h-full w-full overflow-hidden rounded-xl border border-panvas-border-strong bg-panvas-bg-elevated text-panvas-text-primary shadow-glass"
    ...
    <div className="flex items-center gap-2 p-3">
      <button type="button" className="panvas-icon-control h-8 w-8 shrink-0" disabled={!fileId} onClick={() => void toggle()} aria-label={state.status === 'playing' ? 'Pause voice note' : 'Play voice note'} title={state.status === 'playing' ? 'Pause voice note' : 'Play voice note'}>{state.status === 'playing' ? <Pause size={14}/> : <Play size={14}/>}</button>
      <div className="min-w-0 flex-1"><input type="range" aria-label="Voice note playback position" title="Voice note playback position" className="block w-full accent-panvas-accent-blue" min={0} max={Math.max(1,duration)} value={Math.min(state.elapsedMs, Math.max(1, duration))} disabled={!duration} onChange={event => player.current?.seek(Number(event.target.value))}/><div className="flex justify-between text-[10px] tabular-nums text-panvas-text-tertiary"><span>{formatAudioTime(state.elapsedMs)}</span><span>{formatAudioTime(duration)}</span></div></div>
    </div>
