...
import { Mail, Lock, Eye, EyeOff, UserPlus, User, AlertCircle, Loader2, Check } from 'lucide-react';
import { AuthLayout } from './AuthLayout';
import { useAuthStore } from '@/stores/authStore';
import { signupSchema, type SignupInput, getPasswordStrength } from '@/lib/validation/auth';

// OAuth icons (same as LoginPage)
function GoogleIcon() {
  return (
...
export function SignUpPage() {
  ...
  return (
    <AuthLayout title="Create your account" subtitle="Start building your visual research workspace.">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
        ...
        <div className="flex items-center gap-3 py-1">
          ...
          <span className="text-[11px] text-[#525252] font-medium">or sign up with email</span>
          <div className="flex-1 h-px bg-white/6" />
        </div>

        {/* Full Name */}
        <div>
          <label htmlFor="signup-name" className="block text-xs font-medium text-[#A3A3A3] mb-1.5">
            Full name
          </label>
          ...
          {errors.fullName && (
            <p id="signup-name-error" className="mt-1 text-xs text-[#f43f5e]">{errors.fullName.message}</p>
          )}
        </div>

        {/* Email */}
        <div>
          <label htmlFor="signup-email" className="block text-xs font-medium text-[#A3A3A3] mb-1.5">
            Email address
          </label>
          <div className="relative">
          ...
          {errors.email && (
            <p id="signup-email-error" className="mt-1 text-xs text-[#f43f5e]">{errors.email.message}</p>
          )}
        </div>

        {/* Password */}
        <div>
          <label htmlFor="signup-password" className="block text-xs font-medium text-[#A3A3A3] mb-1.5">
            Password
          </label>
          <div className="relative">
          ...
          {errors.password && (
            <p id="signup-password-error" className="mt-1 text-xs text-[#f43f5e]">{errors.password.message}</p>
          )}
        ...
        {/* Confirm Password */}
        <div>
          <label htmlFor="signup-confirm-password" className="block text-xs font-medium text-[#A3A3A3] mb-1.5">
            Confirm password
          </label>
          <div className="relative">
            <Lock size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#525252]" />
          ...
          {errors.confirmPassword && (
            <p id="signup-confirm-error" className="mt-1 text-xs text-[#f43f5e]">{errors.confirmPassword.message}</p>
          )}
        </div>
        ...
        <div className="flex items-start gap-2.5 pt-1">
          <input
            ...
            type="checkbox"
            id="signup-terms"
            className="mt-0.5 w-4 h-4 rounded border-white/15 bg-[#121212] text-[#E8E8E8] focus:ring-[#E8E8E8]/30 focus:ring-offset-0 cursor-pointer"
            aria-invalid={!!errors.acceptTerms}
          />
          <label htmlFor="signup-terms" className="text-xs text-[#737373] cursor-pointer leading-relaxed">
            I agree to the{' '}
            <a href="#" className="text-[#A3A3A3] hover:text-[#E8E8E8] underline transition-colors">Terms of Service</a>
            {' '}and{' '}
            <a href="#" className="text-[#A3A3A3] hover:text-[#E8E8E8] underline transition-colors">Privacy Policy</a>
          </label>
        </div>
        {errors.acceptTerms && (
          <p className="text-xs text-[#f43f5e] -mt-2">{errors.acceptTerms.message}</p>
        )}
        ...
        {/* Login link */}
        <p className="text-center text-xs text-[#525252] pt-1">
